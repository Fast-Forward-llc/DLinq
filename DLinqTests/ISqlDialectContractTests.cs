...
namespace DLinqTests
{
    ...
    public class ISqlDialectContractTests
    {
        ...
        public void DummyDialect_ImplementsAllMethods()
        {
            ...
            Assert.AreEqual("INSERT INTO T", dialect.InsertStatement("T", new List<string>(), new List<string>(), new DLinq.InsertOptions()));
            Assert.AreEqual("UPDATE T", dialect.UpdateStatement("T", new { A = 1 }, new { B = 2 }, new DLinq.UpdateOptions(), new List<(string, object)>()));
            Assert.AreEqual("DELETE FROM T", dialect.DeleteStatement("T", new { A = 1 }));
